...
class WhaleBot:
    ...
    async def _stop_loss_loop(self) -> None:
        ...
        Resolution runs every 10s (every tick) to eliminate the window where a
        stop-loss can fire before resolution is caught.  Both checks run at 10s.
        ...
        while self._running:
            try:
                ...
                # ── Resolution check FIRST (every tick) ──────────────
                try:
